...
use Oro\Bundle\ContactBundle\Entity\Manager\AccountManager;
...
class AccountManagerTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @var string
     */
    protected $class = 'Oro\Bundle\ContactBundle\Entity\Account';
    ...
    {
        /** @var Account $account */
        $account = $this->getMock('Oro\Bundle\ContactBundle\Entity\Account');
        $this->om->expects($this->once())
            ->method('persist')
            ->with($account);
    ...
    public function testGetListQuery()
    {
        ...
        $queryBuilder->expects($this->once())
            ->method('from')
            ->with('OroContactBundle:Account', 'a')
            ->will($this->returnSelf());
        $queryBuilder->expects($this->once())
            ->method('orderBy')
            ->with('a.id', 'ASC')
